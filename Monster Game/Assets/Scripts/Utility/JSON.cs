...
namespace TCG.Utility
{
    ...
    public abstract class JSONNode
    {
        ...
        public virtual string QValue { get { return ""; } set { } }

        public virtual string Value
        {
            get { return QValue.Replace("\"", ""); }
        }
        ...
        public virtual float AsFloat
        {
            ...
            {
                float v = 0.0f;
                if (float.TryParse(QValue, out v))
                    return v;
                return 0.0f;
            }
            set
        ...
        public virtual double AsDouble
        {
            ...
            {
                double v = 0.0;
                if (double.TryParse(QValue, out v))
                    return v;
                return 0.0;
            }
            set
        ...
        public virtual bool AsBool
        {
            ...
            {
                bool v = false;
                if (bool.TryParse(QValue, out v))
                    return v;
                return !string.IsNullOrEmpty(QValue);
            }
            set
            {
                QValue = (value) ? "true" : "false";
    ...
    public static class JSON
    {
        ...
        public static JSONNode Jsonify(object obj)
        {
            return Parse(Stringify(obj));
        }
        public static JSONNode Jsonify(object obj, string arrOfProperties)
        {
            return Parse(Stringify(obj, arrOfProperties));
        }
        public static JSONNode Jsonify(object obj, string[] arrOfProperties)
        {
            return Parse(Stringify(obj, arrOfProperties));
        ...
        {
            var arrOfProperties = obj.GetType().GetProperties(BindingFlags.Public).Select(t => t.Name).ToArray();

            return Stringify(obj, arrOfProperties);
        }

        public static string Stringify(object obj, string arrOfProperties)
        {
            var arr = arrOfProperties.Split(',');

            return Stringify(obj, arr);
        }

        public static string Stringify(object obj, string[] arrOfProperties)
        {
            var listOfProperties = new List<string>(arrOfProperties);
            var allProperties = obj.GetType().GetProperties();
            var properties = allProperties.Where(p => listOfProperties.Contains(p.Name)).ToArray();
            var output = new StringBuilder("{");

            for (var i = 0; i < properties.Length; i++)
            {
                var prop = properties[i];
                output.AppendFormat("\"{0}\" : \"{1}\"", prop.Name, prop.GetValue(obj, null));
